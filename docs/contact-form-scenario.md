...
| Maintainer | Re-run `python -m screencast run scripts/screencasts/contact_form.robot` any number of times. | `Prepare Fixtures` deletes and recreates its own **Contact us** page (Reception creates it on camera and so owns/publishes it fresh each run) and clears all Operaton deployments first, so Cockpit starts clean on every run. |
...
| `output.webm` | `python -m screencast compose`'s Full HD composite: Plone alone at first, Operaton introduced as a PIP during the first submission, the refreshed definition and second instance views brought front briefly, then the reviewed instance kept as a PIP |
...
```sh
python -m screencast compose var/screencasts/contact_form/<take>/
python -m screencast verify var/screencasts/contact_form/<take>/
```
