...
Or do all of it in one command: `make demo-stack STORY=contact_form` brings up the
...
The worker listens on port 8001 (`make serve PORT=...` changes it). Each example
...
| Maintainer | Re-run `playwright-python -m screencast run scripts/screencasts/contact_form.robot` any number of times. | `Prepare Fixtures` deletes and recreates its own **Contact us** page (Reception creates it on camera and so owns/publishes it fresh each run) and clears all Operaton deployments first, so Cockpit starts clean on every run. |
...
Written to `var/screencasts/contact_form/<take>/` (gitignored -- nothing here
is committed, only regenerated by re-running the story):
...
| `output.webm` | `playwright-python -m screencast compose`'s Full HD composite: Plone alone at first, Operaton introduced as a PIP during the first submission, the refreshed definition and second instance views brought front briefly, then the reviewed instance kept as a PIP |
...
(`var/screencasts/contact_form/<take>/screenshots/`). They differ slightly on
every run, so a run never overwrites the tracked copies in `docs/` by
accident; after a take you are happy with,
`make promote-screenshots STORY=contact_form` copies the latest ones into `docs/`
...
can still exit 0 -- `verify` is what actually checks it, so run it after
every take:

```sh
playwright-python -m screencast compose var/screencasts/contact_form/<take>/
playwright-python -m screencast verify var/screencasts/contact_form/<take>/
```

`verify` derives its contact-sheet sampling rate from the take's own measured
duration, so there is no per-scenario `tile=RxC` rate to hand-tune or
recompute here -- see `scripts/screencast/verify.py`. The six recorded turns
are one Reception setup turn, two Visitor turns, two Reception decision
turns, and one Specialist turn; `report.json`'s findings say which check
failed and why on a broken take.

## Cleanup

`Prepare Fixtures` deletes and recreates "Contact us" at the start of every
run, and clears all existing Operaton deployments before deploying its four
assets. It does not delete the `reception` or `specialist` accounts.
Mailpit's own inbox is not cleared between runs -- expect earlier runs' reply
emails to still be listed there. To remove a take, delete its
`var/screencasts/contact_form/<take>/` directory; to remove the
doc-illustration screenshots, delete the `contact-form-*.png` files in
`docs/`.

## Implementation checklist

The implementation is now in place. Re-run the story against the shared
services whenever the process or composition changes.

> The Playwright script this checklist describes
> (`scripts/scenarios/e2e_contact_form.py`) has been ported to
> `scripts/screencasts/contact_form.robot`, built on `scripts/screencast/`
> (the generic engine) and `scripts/screencasts/resources/bpmproxy.resource`
> -- see collective/collective.bpmproxy#10. The lessons below (the
> Patternslib toggle race, `paste_text()` for free-text fields, clearing
> stale deployments first) are still the reasons the story is built the way
> it is; only the file layout and the "Playwright script" framing are
> out of date.
